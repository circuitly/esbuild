...
async function installFromNPM(name: string, subpath: string): Promise<string> {
  ...
  const url = `${npmRegistry}/${name}/-/${name.replace("@circuitly/esbuild-", "")}-${version}.tgz`
...
async function install(): Promise<string> {
  ...
  const knownWindowsPackages: Record<string, string> = {
    'x86_64-pc-windows-msvc': '@circuitly/esbuild-win32-x64',
  }
  const knownUnixlikePackages: Record<string, string> = {
    // These are the only platforms that Deno supports
    'aarch64-apple-darwin': '@circuitly/esbuild-darwin-arm64',
    'aarch64-unknown-linux-gnu': '@circuitly/esbuild-linux-arm64',
    'x86_64-apple-darwin': '@circuitly/esbuild-darwin-x64',
    'x86_64-unknown-linux-gnu': '@circuitly/esbuild-linux-x64',

    // These platforms are not supported by Deno
    'aarch64-linux-android': '@circuitly/esbuild-android-arm64',
    'x86_64-unknown-freebsd': '@circuitly/esbuild-freebsd-x64',
    "x86_64-alpine-linux-musl": '@circuitly/esbuild-linux-x64',
  }
